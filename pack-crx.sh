...
#   - cleanor-gif-extension.zip                          (plain ZIP — use this for the FIRST publish)
#   - <signing-dir>/cleanor-gif-extension-<version>.crx  (signed, only once Verified CRX Uploads is on)
#
...
FILES=(manifest.json background.js popup.html popup.css app.js worker.js gif-core.js icons vendor)

VERSION="$(python3 -c "import json;print(json.load(open('manifest.json'))['version'])")"
echo "Cleanor GIF Toolkit · v$VERSION"

# --- 1) plain ZIP ------------------------------------------------------------
rm -f cleanor-gif-extension.zip
zip -r -X cleanor-gif-extension.zip "${FILES[@]}" -x '*.DS_Store' >/dev/null
echo "✓ zip:  $ROOT/cleanor-gif-extension.zip"

[[ "${1:-}" == "--zip-only" ]] && exit 0

# --- 2) signed CRX (optional; only once a signing key exists) ----------------
KEY="${CLEANOR_GIF_CRX_KEY:-$(cd "$(dirname "$0")/.." && pwd)/003-gif-toolkit-signed/cleanor-gif-ext.pem}"
CHROME="${CHROME_BIN:-/Applications/Google Chrome.app/Contents/MacOS/Google Chrome}"

if [[ ! -f "$KEY" ]]; then
  echo "ℹ no signing key at: $KEY — that's expected for the first publish (upload the .zip)."
  echo "  Enable Verified CRX Uploads later, then set CLEANOR_GIF_CRX_KEY to sign updates."
  exit 0
fi
if [[ ! -x "$CHROME" ]]; then
...
STAGE="$(mktemp -d)/cleanor-gif-extension"
mkdir -p "$STAGE"
cp -R "${FILES[@]}" "$STAGE/"
find "$STAGE" -name '.DS_Store' -delete

"$CHROME" --pack-extension="$STAGE" --pack-extension-key="$KEY" --no-message-box >/dev/null 2>&1 || true
SRC_CRX="$(dirname "$STAGE")/cleanor-gif-extension.crx"
if [[ -f "$SRC_CRX" ]]; then
  DEST="$(dirname "$KEY")/cleanor-gif-extension-$VERSION.crx"
  mv -f "$SRC_CRX" "$DEST"
  rm -rf "$(dirname "$STAGE")"
